...
for train_index, test_index in cross_val.split(X):
    ...
    datagen_val_x.fit(X_train, augment=True, seed=datagen_seed)
    datagen_val_y.fit(Y_train, augment=True, seed=datagen_seed)
    ...
    xvalgen = datagen_val_x.flow(X_val, seed=datagen_seed, batch_size=16)
    yvalgen = datagen_val_y.flow(Y_val, seed=datagen_seed, batch_size=16)
